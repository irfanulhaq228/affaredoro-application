import { View, Text, StyleSheet, ScrollView, Alert, TouchableOpacity, Modal, TouchableWithoutFeedback, FlatList, TextInput, Image, ToastAndroid, Linking } from 'react-native';
import React, { useCallback, useEffect, useReducer, useRef, useState } from 'react';
import { COLORS, SIZES, FONTS, icons, images as Images } from '../constants';
...
const UploadItem = () => {
  ...
  const handleOnboard = async (stripeAccountId:any) => {
    ...
    try {
      ...
      const refreshUrl = 'https://affaredoro.com/stripe/onboard/refresh';
      const returnUrl  = 'https://affaredoro.com/stripe/onboard/return';
  ...
  const submit = async () => {
    ...
    if (!name) {
      ToastAndroid.show('Must add product title', 2000)
      setIsLoading(false)
      return
    }

    if (!description) {
      ToastAndroid.show('Must add product description', 2000)
      setIsLoading(false)
      return
    }
    ...
    if (images.length === 0) {
      ToastAndroid.show('Must upload atleast one image', 2000)
      setIsLoading(false)
      return
    }
    ...
    if (!sel) {
      ToastAndroid.show('Must select Category', 2000)
      setIsLoading(false)
      return
    }

    if (!price) {
      ToastAndroid.show('Must add product price', 2000)
      setIsLoading(false)
      return
    }
  ...
  return (
    <>
      ...
      <SafeAreaView style={[styles.area, { backgroundColor: dark ? COLORS.dark1 : COLORS.white, }]}>
        ...
        <ShowCategoryModal show={showBrand} setShow={setShowBrand} title={'Select Brand'} data={brandOptions} value={'_id'} name={'name'} setSel={setSelBrand} sel={selBrand} showSearch={true} onPress={() => {
          setShowBrand(false)
        }} />
